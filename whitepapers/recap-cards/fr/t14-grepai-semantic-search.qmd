...
Grep natif (ripgrep) est idéal quand on connaît le texte exact à trouver. Grepai prend le relai quand on cherche "le code qui gère X" sans connaître les noms exacts des fonctions ou des variables. Il s'appuie sur des embeddings locaux via Ollama et le modèle `nomic-embed-text`, ce qui maintient cette étape d’indexation en local. Les résultats transmis à Claude peuvent néanmoins entrer dans le contexte du fournisseur configuré.

**Mesurer la valeur** : comparer précision de recherche, taille du contexte retourné et coût par tâche acceptée sur des recherches représentatives.
...
|-----------|-----------------|
| Pattern exact connu (`"authenticate"`) | Grep natif |
| Recherche par concept ou intention | Grepai |
