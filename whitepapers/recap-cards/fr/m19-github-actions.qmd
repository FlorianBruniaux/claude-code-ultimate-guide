...
|---------|----------------|-----------|
| OAuth abonnement | Consomme le quota du plan ; limites applicables | Compte éligible et token OAuth |
| `ANTHROPIC_API_KEY` | Facturation API au token | Clé API Anthropic |

Choisissez l’authentification selon le compte et le volume de travail. Une revue sur abonnement consomme le quota ; aucune méthode ne garantit un coût fixe par revue.
...
`fetch-depth: 0` récupère tout l’historique. L’action actuelle accepte `prompt` et `claude_args` ; la restriction CLI des outils garde cet exemple en lecture seule. Voir les [entrées officielles de l’action](https://github.com/anthropics/claude-code-action/blob/main/action.yml).
