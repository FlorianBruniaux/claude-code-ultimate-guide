...
| Enterprise, production | Critique | Moindre privilège, isolation et vérification d’intégrité |
...
**Règle de base sur les permissions :** accorder uniquement ce que la tâche nécessite. Pour une analyse, limiter l’agent avec `tools: Read, Grep, Glob` ou `--tools "Read,Grep,Glob"`. Une règle allow pré-approuve un outil ; elle ne retire pas les autres.
