...
La façon la plus productive de penser à Claude Code : un développeur junior très rapide, avec une connaissance encyclopédique des patterns, mais sans le jugement qui vient de l'expérience projet. Il peut produire rapidement du code tout en commettant des erreurs d’architecture nécessitant une relecture attentive.
...
|-------|---------------------|--------|
| Renommage, formatage | Élevé | Vérifier le diff obtenu |
...
Une erreur initiale peut se propager aux changements suivants. Les reprises dépendent des dépendances et du moment où la validation la détecte.
...
```
/rewind    # Ouvrir le choix des points de contrôle
           # Choisir quoi restaurer puis vérifier le résultat
```
