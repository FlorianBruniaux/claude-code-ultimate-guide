...
Dans cette organisation choisie, l’orchestrateur se concentre sur la planification et la délégation ; c’est une convention de rôle, pas une restriction d’outil. Son rôle est d'allouer le travail, de débloquer les dépendances, et de présenter une réponse unifiée.
...
| Planificateur | Read, Glob, Grep ; écritures du plan autorisées séparément |
...
Prérequis : Claude Code v2.1.32+, modèle accessible au compte (différent selon le coéquipier si besoin). Git est nécessaire à l’isolation par worktree, pas à la messagerie des tâches. `Shift+Down` passe entre agents dans le même processus.
