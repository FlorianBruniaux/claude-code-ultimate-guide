...
> **CC 2.1.3** : Commandes personnalisées et skills partagent leur comportement d’invocation. Les fichiers `.claude/commands/*.md` restent pris en charge. Ajouter `disable-model-invocation: true` pour une invocation manuelle uniquement.
...
Un skill sans `disable-model-invocation` peut être invoqué par le modèle lorsque sa description correspond à la tâche.
...
Les plugins ajoutent des capacités externes. Ajouter un catalogue avec `/plugin marketplace add <source>`, puis installer le plugin choisi avec `/plugin install <name>@<marketplace>`.
