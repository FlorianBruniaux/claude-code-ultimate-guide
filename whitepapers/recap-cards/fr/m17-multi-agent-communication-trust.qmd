...
Les contextes restent isolés : l’agent 2 ne voit pas la conversation de l’agent 1. Seuls les messages explicites traversent la frontière, ce qui implique de formuler les transmissions de façon précise et concise.
...
## Coordination par tâches partagées

Les Agent Teams stockent leurs tâches dans `~/.claude/tasks/<team-name>/`. Le verrouillage de fichiers évite les prises en charge simultanées d’une tâche ; il ne verrouille pas les fichiers source et ne fusionne pas le code. Utilisez `TaskList` et `TaskUpdate` pour coordonner le travail.
...
Attribuez des fichiers distincts ou des worktrees à chaque agent. Les messages transportent l’avancement et les résultats ; les conflits source nécessitent toujours une revue.
