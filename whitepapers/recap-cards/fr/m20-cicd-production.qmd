...
Ce flag contourne les demandes de permission. Les tâches non interactives peuvent aussi utiliser des outils pré-approuvés et un ensemble d’outils restreint. Mais il supprime aussi la dernière ligne de défense contre les actions destructives. Son utilisation n'est acceptable que dans un container éphémère isolé, jamais sur une machine partagée persistante.
...
Cet exemple donne accès en lecture au code source. Identifiants, isolation du runner et déclencheurs du workflow nécessitent une revue séparée. Une microVM ajoute une frontière de noyau ; aucune virtualisation n’élimine tous les risques d’évasion ou d’accès aux données.
...
Utiliser `--tools` pour limiter les outils disponibles et `--allowedTools` pour pré-approuver les appels autorisés. Transmettre ces options via `claude_args` dans l’action officielle.
...
Tout le reste est refusé par défaut. Cela restreint les destinations, mais un service autorisé peut recevoir des données sensibles. Examiner identifiants, périmètre des requêtes et données accessibles à l’agent.
