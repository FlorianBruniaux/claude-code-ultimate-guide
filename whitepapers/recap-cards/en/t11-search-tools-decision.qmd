...
Glob returns a list of paths without reading their content. Use it to narrow the file set before reading content.
...
Grep uses ripgrep under the hood, making it suited to exact text searches. It reads files partially to extract only the matching lines, without loading the full content.
...
Use Grep for exact text searches. Do not escalate to Agent or Bash if the pattern is known.
