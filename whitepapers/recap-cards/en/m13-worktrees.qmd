...
Each Claude instance indexes its own worktree and maintains an independent context. Files are separated, but ports, databases and external services may still be shared.
...
| `CLAUDE.local.md` (add to `.gitignore`) | Personal instructions in this worktree |
...
**Recommended approach**: install dependencies independently in each worktree, using the package manager cache to reduce downloads. A shared `node_modules` symlink couples branches and is unsafe when lockfiles or install state differ.

```bash
# From the new worktree, using the repository's package manager
pnpm install --frozen-lockfile
```

Review any custom `/git-worktree` helper before using its dependency-sharing options.
