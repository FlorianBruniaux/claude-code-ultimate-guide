...
- **Landing rebuild dispatch now reports why it failed and how to fix it** (`.github/workflows/trigger-landing-deploy.yml`, `.github/workflows/index-integrity.yml`, `scripts/test-trigger-landing-deploy.py`, `docs/workflows/landing-sync.md`): the cross-repository dispatch failed with a bare HTTP 401 and the run could not distinguish an expired `LANDING_DEPLOY_TOKEN` from an absent one, because an empty token sends an unauthenticated request that 401s identically. Added a preflight that checks the secret is present without logging its value or length, and mapped 401, 403 and 404 to messages naming the next action; any other status is rethrown rather than swallowed, so an API outage can never exit green and leave the landing serving stale content. The job now declares `permissions: {}`, since it needs no access to this repository, and accepts `workflow_dispatch` so a rotation can be verified without pushing to `guide/`. Documented credential ownership, the fine-grained token scope (Actions: write on the landing repo only), the rotation procedure and a failure-reading table, recording no secret value. Covered by 35 tests that execute the preflight and run the dispatch script against a mocked API; they report 20 named failures against the pre-fix workflow. Rotating the credential itself remains with the repository owner.
