...
- **README: Star History chart migrated to a working mirror** (`README.md`): The star-history.com SVG endpoint stopped rendering under GitHub's stargazer API restrictions, so the live star growth chart before the Contributing section was moved to star-history.dera.page. The wrapping link now uses the mirror's hash-based URL with the same parameters.
