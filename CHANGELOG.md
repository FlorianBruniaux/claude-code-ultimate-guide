...
- **README.md and AGENTS.md swept for the em dash violation in `~/.claude/ANTI_AI.md`** (`README.md`, `AGENTS.md`): a mechanical scan reusing the `anti-ai-markers.sh` hook's exact patterns found 114 em dashes in README.md and 2 in AGENTS.md, all in the `label — description` bullet/heading convention; all other hook-checked markers (stereotyped EN phrases, redundant modal stacks, stereotyped openings, vague link labels, hollow evidentiality) were already at zero across both files. 112 of README's occurrences converted mechanically to a colon (`**Label** — text` → `**Label**: text`); 5 lines where that produced an awkward adjacent double colon (an existing `**Bold**:` already present before the converted dash) were hand-fixed to a comma or a restructured clause instead. CHANGELOG.md itself carries 587 more em dashes in dense technical entries where a blanket colon substitution would risk misreading nested clauses; left untouched pending a scope decision, since the hook already blocks new em dashes in any future edit to this file.
