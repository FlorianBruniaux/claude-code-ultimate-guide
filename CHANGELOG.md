...
- **OrcaRouter documented as a managed gateway** (`guide/ops/api-gateway.md`): added a new §8 covering OrcaRouter as a first-class managed alternative to the self-hosted LiteLLM setup, connecting Claude Code through `ANTHROPIC_API_URL=https://api.orcarouter.ai` with `orcarouter/auto` routing and Anthropic-protocol model ids (`anthropic/claude-*`). The page's section numbers were renumbered accordingly (Portkey moved to §9, "What the Gateway Does Not Cover" to §10), and `machine-readable/reference.yaml` plus its `mcp-server/content/` mirror were updated in both the `deep_dive` keys and the file's section map, verified byte-identical after the edit. Live-verified against the gateway's Anthropic Messages endpoint (`/v1/messages` → 200) before writing.
