...
- **The domain allowlist is inert under auto-allow mode, and the guide said the opposite** (`guide/security/sandbox-native.md`): the section described a `policy` field that does not exist and called allowlist mode the default. The real switch is `strictAllowlist`, default `false`, and the permissive mode it selects relies on a permission prompt that `autoAllowBashIfSandboxed` answers for you. Run the pair together and every host is reachable: `example.com` and `api.openai.com` both returned HTTP 200 from a sandboxed command whose `allowedDomains` held 23 unrelated entries. Audits should look for the pair rather than for the presence of a domain list. Also notes that `github.com` does not cover `codeload.github.com`, where npm and pnpm fetch git dependencies and tarballs.
