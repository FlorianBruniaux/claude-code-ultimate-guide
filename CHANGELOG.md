...
- **Agent Harness Map enrichment specified and implementation-planned** (`docs/superpowers/specs/2026-08-27-agent-harness-landscape-enrichment-design.md`, `docs/superpowers/plans/2026-08-27-agent-harness-landscape-enrichment.md`): defined a reproducible two-depth catalog covering all 160 Best of Agent Harnesses projects and full evidence profiles for the 86 deep-dive entries; required a verified project URL in every table row, including Devin; separated confirmed, claimed, unknown, and not-applicable feature states; designed README and GitHub-metadata collection, constrained semantic extraction with human review, deterministic Markdown generation, three Bold Guy visuals, guide and landing synchronization, dirty-worktree isolation, tests, rollback, and production verification. This entry records planning artifacts only; the dataset, page expansion, visuals, and landing changes are not implemented yet.
...
- **Agent harness landscape expanded and clarified** (`guide/core/agent-harness.md`, `guide/core/glossary.md`, `guide/ecosystem/agent-harness-landscape.md`, `guide/ecosystem/agentic-tools.md`, `guide/ultimate-guide.md`): separated model, runtime harness, repository harness, and orchestrator into four explicit layers; expanded the curated runtime table from 25 to 33 products; added a separate seven-product orchestrator table; moved the full DeepSeek Harness profile, setup command, architecture, maturity limits, and prompt-injection evidence into the agent tools guide; and aligned navigation, glossary, machine-readable indexes, MCP content, and both `llms.txt` entry points.

- **Monitor, WebSocket and safe Codex delegation documented** (`guide/workflows/monitor-event-delegation.md`, `guide/core/tools-reference.md`, `guide/core/settings-reference.md`, `guide/ultimate-guide.md`): added a canonical workflow separating Monitor command source (v2.1.98+), native WebSocket source (v2.1.195+), experimental plugin monitors (v2.1.105+), MCP Channels (v2.1.80+, preview), and Routines' separate cloud sessions. The GitHub-to-Monitor path now requires signature validation, repository/event allowlisting, schema validation, `X-GitHub-Delivery` deduplication, and a narrow WebSocket payload before a local Monitor sees it. Codex starts read-only with `codex exec`; an explicit human or policy gate is required before a separate isolated `--sandbox workspace-write` task. Raw event bodies and comments are documented as data, never instructions or authorization. Added cross-links from cross-session messaging, generic event-driven agents, GitHub Actions, navigation, and machine-readable indexes; both `llms.txt` files now track Claude Code v2.1.246 and remain identical.
