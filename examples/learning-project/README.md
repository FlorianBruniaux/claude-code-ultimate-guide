...
| 06 | [Hooks and events](../../guide/learning-path/06-hooks.md) | Inspect the [PreToolUse hook](.claude/hooks/release-guard.mjs), its [configuration](.claude/settings.json), and the pass, fail, and malformed fixtures. | `npm run hook:fixtures` with all four cases passing. |
...
Every check needs `status: "pass"` and a non-empty `evidence` string. The version must use `MAJOR.MINOR.PATCH`. A duplicate name is a failure because later evidence must not shadow an earlier result.
...
The npm dry run builds a package manifest without downloading dependencies or publishing an artifact. The [Dockerfile](Dockerfile) supplies an additional deployable form. Building it may require the `node:22-alpine` base image from a registry, so a passing Node.js test run does not prove the image builds or runs on another host.
...
The release guard blocks every `npm publish` and `docker push` tool call and directs the learner back to local verification. It does not auto-unblock after a passing check. The hook parses its input without running the requested command and denies malformed input. It is a narrow teaching control, not a shell parser or a general command firewall. Apply the repository's [security hardening guidance](../../guide/security/security-hardening.md) before adapting hooks to production.

Publishing also needs a channel owner, destination credentials, and an explicit decision. The [guide distribution workflow](https://github.com/FlorianBruniaux/claude-code-ultimate-guide/blob/main/docs/workflows/guide-distribution.md) separates those external actions from local packaging checks.
