...
- A rubric with weights, observable anchors, passing threshold, tie-breaker, candidate count, and stop rule.
...
1. Freeze the contract in the proof log. Default to three candidates. Use five only when the expected improvement justifies the additional generation, scoring, and verification cost.
2. Generate each candidate from the same frozen contract. Do not reveal candidate text, scores, or private reasoning across generators. Use opaque IDs such as `C-01`.
3. Preserve each candidate as a separate artifact. For code, use isolated diffs or worktrees from the same base revision.
4. Blind provenance and presentation order for scoring when practical. Apply the fixed rubric, record criterion-level evidence, and disqualify mandatory failures.
...
- Stop at the declared limit or after a candidate passes every mandatory criterion and its executable checks. Do not keep sampling until an answer feels persuasive.
...
BEST-OF-N RESULT
Task: <scope>
Contract: <rubric version, candidate count, stop rule>
